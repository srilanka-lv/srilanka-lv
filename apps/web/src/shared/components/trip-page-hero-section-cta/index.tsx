  ...
  <div className={clsx(className, tripPageHeroSectionCtaStyle)}>
    <button
      ...
      onClick={async () => {
        ...
        // Adds the trip to the celoarmariku.lv (Shopify) cart with the 400€
        // Downpay deposit plan applied. `id` is the variant of the current
        // departure and `selling_plan` its deposit plan: both change if the
        // partner recreates the product or the plan, which breaks this link.
        // The shorter /cart/<id>:<qty> permalink form must NOT be used here:
        // it drops the selling plan and charges the full trip price.
