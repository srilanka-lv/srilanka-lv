...
export const generateMetadata = (): Promise<Metadata> => buildPageMetadata(PAGES.LV.INFO_VISA);
...
const NextHolidayInSriLankaWhatToDoPage: FunctionComponent = async () => {
  ...
  const post = await repository.query(pagesBySlugQuery, {
    slug: PAGES.LV.INFO_VISA,
  });

  return <StaticPageLayout href={`/${PAGES.LV.INFO_VISA}`} body={post?.body} page={post} />;
};
